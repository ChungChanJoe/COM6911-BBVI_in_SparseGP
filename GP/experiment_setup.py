...
class ExperimentSetup:
    ...
    @staticmethod
    def istanbul_mul_data(config):
        method = config['method']
        sparsify_factor = config['sparse_factor']
        np.random.seed(12000)
        data = DataSource.istanbul_mul_data()
        d = data[config['run_id'] - 1]
        names = []
        Xtrain = d['train_X']
        Ytrain = d['train_Y']
        ...
        Ytest = d['test_Y']
        name = 'istanbul'
        kernel = ExperimentSetup.get_kernels(Xtrain.shape[1], 2, True)
        # gaussian_sigma = np.var(Ytrain)/4 + 1e-4
        gaussian_sigma = 1.0
        # number of inducing points
        num_inducing = int(Xtrain.shape[0] * sparsify_factor)
